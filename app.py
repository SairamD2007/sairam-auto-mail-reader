...
# Define permissions context
SCOPES = "https://www.googleapis.com/auth/gmail.readonly"

# --- PRODUCTION URL DISCOVERY ROUTINE ---
# Dynamically resolves the address space to ensure parity across desktop and mobile browsers
if "credentials" not in st.session_state:
    try:
        # Attempts to read live deployment path from header arrays automatically
        host_origin = st.context.headers.get("Host", "localhost:8501")
        if "localhost" in host_origin:
            REDIRECT_URI = f"http://{host_origin}/"
        else:
